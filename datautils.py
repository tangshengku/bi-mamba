...
def get_loaders(name, tokenizer, nsamples=128, seed=0, seqlen=2048):
    ...
    if name == 'simple':
        loaders = get_simple(nsamples, seed, seqlen, tokenizer)
    if name == 'random':
        loaders = get_random(nsamples, seed, seqlen, tokenizer)

    return loaders

def get_simple(nsamples, seed, seqlen, tokenizer):
    testdata = "\n\n".join([
        'I like'+'e'*2048,
        'I am'+'m'*2048,
        'he is'+'s'*2048
    ...
    testenc = tokenizer(testdata, return_tensors='pt')

    return None, testenc


def get_random(nsamples, seed, seqlen, tokenizer):
    testdata = ''.join([chr(random.randint(0, 255)) for _ in range(2048*3)])
...
if __name__ == '__main__':
    ...
    _, testenc = get_loaders('wikitext2', tokenizer, nsamples=128, seed=0, seqlen=2048)
